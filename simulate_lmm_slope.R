...
no_ranef_with_slope <- list(intercept = list(raters = .0, items = .0),
...
simulate_lmm_with_slope <- function(n_raters = 10, 
                         n_items = 10, 
                         n_conditions = 2,
  ...
  if(missing(condition_labels)){
    conditions_labels <- as.character(n_conditions)
  }
